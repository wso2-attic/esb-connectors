...
public class GoogleSpreadsheetUpdateWorksheetMetadata extends AbstractConnector {
	...
	public void connect(MessageContext messageContext) throws ConnectException {
		try {
			...
			if(gssWorksheet.updateWorksheetMetadata(worksheetOldName, worksheetNewName, rowCount, columnCount)) {
			...
			messageContext.getEnvelope().getBody().addChild(searchResult);
                ...
                ConnectException connectException = new ConnectException("Cannot update metadata. Spreadsheet with the given name is not available.");
                log.error("Error occured: " + connectException.getMessage(), connectException);
                GoogleSpreadsheetUtils.storeErrorResponseStatus(messageContext, connectException);
            }
