...
public class GoogleSpreadsheetWorksheet {
    ...
    public WorksheetEntry getWorksheetByTitle(String worksheetName) throws IOException, ServiceException {
	   ...
	   *         Spreadsheets service.
	   */
	  public boolean deleteWorksheet(String title) throws IOException,
	      ServiceException {
	    WorksheetFeed worksheetFeed = service.getFeed(worksheetFeedUrl,
	        WorksheetFeed.class);
	    for (WorksheetEntry worksheet : worksheetFeed.getEntries()) {
	   ...
	   *         Spreadsheets service.
	   */
	  public boolean updateWorksheetMetadata(String oldTitle, String newTitle, int rowCount,
	    ...
	    for (WorksheetEntry worksheet : worksheetFeed.getEntries()) {
	      ...
	      if (currTitle.equals(oldTitle)) {
	        ...
	        worksheet.update();
	        log.debug("Worksheet updated.");
	        return true;
	      }
	    }

	    // If it got this far, the worksheet wasn't found.
	    log.debug("Worksheet not found: " + oldTitle);
          return false;
	  }
