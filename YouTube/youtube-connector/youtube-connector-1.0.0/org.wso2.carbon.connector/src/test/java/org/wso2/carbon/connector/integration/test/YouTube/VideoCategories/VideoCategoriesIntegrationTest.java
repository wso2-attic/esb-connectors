...
public class VideoCategoriesIntegrationTest extends ESBIntegrationTest {
    ...
    @Test(enabled=false, groups = {"wso2.esb"}, description = "YouTube{VideoCategoriesList} integration test.")
    public void testVideoCategoriesListWithMandatory() throws Exception {
        String jsonRequestFilePath = pathToRequestsDirectory + "VideoCategories/VideoCategoriesList.txt";
        String methodName = "getVideoCategories";
        final String jsonString = ConnectorIntegrationUtil.getFileContent(jsonRequestFilePath);
        final String proxyFilePath = "file:///" + pathToProxiesDirectory + "VideoCategories/" + methodName + ".xml";
        String modifiedJsonString = String.format(jsonString, YouTubeConnectorProperties.getProperty("apiUrl"),
                ...
                YouTubeConnectorProperties.getProperty("VideoCategoriesListpart"),
                YouTubeConnectorProperties.getProperty("VideoCategoriesListregionCode"),
                YouTubeConnectorProperties.getProperty("VideoCategoriesListhl"));
        proxyAdmin.addProxyService(new DataHandler(new URL(proxyFilePath)));
        try {
            JSONObject jsonObject = ConnectorIntegrationUtil.sendRequest(getProxyServiceURL(methodName), modifiedJsonString);
            Assert.assertEquals("youtube#videoCategoryListResponse", jsonObject.getString("kind"));
        } finally {
