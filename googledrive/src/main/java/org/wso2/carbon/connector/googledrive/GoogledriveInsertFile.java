 ...
 * <strong>insert</strong> method.
 * 
 * @see https://developers.google.com/drive/v2/reference/files/insert
 */
public class GoogledriveInsertFile extends AbstractConnector implements
		Connector {
	...
	public void connect(MessageContext messageContext) throws ConnectException {
		...
		String fileContentBaseSixtyFour = (String) getParameter(messageContext,
				GoogleDriveUtils.StringConstants.FILE_CONTENT);
		String title = (String) getParameter(messageContext,
				GoogleDriveUtils.StringConstants.TITLE);
		String description = (String) getParameter(messageContext,
				GoogleDriveUtils.StringConstants.DESCRIPTION);
		String parentId = (String) getParameter(messageContext,
				GoogleDriveUtils.StringConstants.PARENT_ID);
		String mimeType = (String) getParameter(messageContext,
				GoogleDriveUtils.StringConstants.MIME_TYPE);
		String fileName = (String) getParameter(messageContext,
				GoogleDriveUtils.StringConstants.FILE_NAME);
		...
		try {
			...
			com.google.api.services.drive.model.File insertedFile = insertFile(
					service, title, description, parentId, mimeType, fileName,
					fileContentBaseSixtyFour, parameters);

			if (insertedFile != null) {

				hashMapForResultEnvelope.put(
						GoogleDriveUtils.StringConstants.FILE,
						insertedFile.toPrettyString());

				insertedFileResult = GoogleDriveUtils
						.buildResultEnvelope(
								GoogleDriveUtils.StringConstants.URN_GOOGLEDRIVE_INSERTFILE,
								GoogleDriveUtils.StringConstants.INSERTED_FILE_RESULT,
	...
	private com.google.api.services.drive.model.File insertFile(Drive service,
			String title, String description, String parentId, String mimeType,
			String filename, String fileContentBaseSixtyFour,
			HashMap<String, String> parameters) {
		try {
			java.io.File writtenFile = GoogleDriveUtils.writeToTempFile(
					GoogleDriveUtils.StringConstants.TEMP_FILE_NAME,
			...
			Files.Insert insertRequest = service.files().insert(body,
					mediaContent);
			...
			temporaryResult = parameters
					.get(GoogleDriveUtils.StringConstants.OCR);
			...
			temporaryResult = EMPTY_STRING;
			temporaryResult = parameters
					.get(GoogleDriveUtils.StringConstants.USE_CONTENT_AS_INDEXABLE_TEXT);
	...
	private HashMap<String, String> getOptionalParams(
		...
		parameters.put(
				...
				(String) getParameter(messageContext,
						GoogleDriveUtils.StringConstants.TIMED_TEXT_TRACKNAME));
		parameters
				.put(GoogleDriveUtils.StringConstants.USE_CONTENT_AS_INDEXABLE_TEXT,
						(String) getParameter(
								messageContext,
								GoogleDriveUtils.StringConstants.USE_CONTENT_AS_INDEXABLE_TEXT));
		parameters.put(
