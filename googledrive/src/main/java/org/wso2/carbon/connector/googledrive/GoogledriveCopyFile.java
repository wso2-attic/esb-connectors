...
/**
 * Class mediator which maps to <strong>/files</strong> endpoint's <strong>copy</strong> method.
 * 
...
public class GoogledriveCopyFile extends AbstractConnector implements Connector {
    ...
    public void connect(MessageContext messageContext) throws ConnectException {
        ...
        try {
            ...
            if (copiedFile != null) {
                ...
                hashMapForResultEnvelope.put(GoogleDriveUtils.StringConstants.FILE, copiedFile.toPrettyString());
                
                copiedFileResult =
                        GoogleDriveUtils.buildResultEnvelope(GoogleDriveUtils.StringConstants.URN_GOOGLEDRIVE_COPYFILE,
                                GoogleDriveUtils.StringConstants.COPIED_FILE_RESULT, true, hashMapForResultEnvelope);
            ...
            copiedFileResult =
                    GoogleDriveUtils.buildResultEnvelope(GoogleDriveUtils.StringConstants.URN_GOOGLEDRIVE_COPYFILE,
                            GoogleDriveUtils.StringConstants.COPIED_FILE_RESULT, false, hashMapForResultEnvelope);
            messageContext.getEnvelope().getBody().addChild(copiedFileResult);
     ...
     */
    private File copyFile(Drive service, String originFileId, HashMap<String, String> parameters) throws IOException {
    
        String copyTitle = "";
        File copiedFile = new File();
        
        if (originFileId != null && !EMPTY_STRING.equals(originFileId)) {
            copyTitle = "Copy of " + (service.files().get(originFileId).execute().getTitle());
        }
