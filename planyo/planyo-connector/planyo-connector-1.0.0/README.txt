...
Steps to follow in setting integration test.
 ...
 2. Set up a new Planyo account by following the wizard in https://www.planyo.com/login/signup.php url.
	Then, follow the steps in https://www.planyo.com/api.php to obtain the API key.

 3. Follow the below mentioned steps for adding valid certificate to access Planyo API over https.

	i) 	 Extract the certificate from browser(Mozilla Firefox) by navigating to https://api.planyo.com
	ii)  Go to new ESB folder and place the downloaded certificate in both "<ESB_HOME>/repository/resources/security/" and "{PLANYO_CONNECTOR_HOME}/planyo-connector/planyo-connector-1.0.0/org.wso2.carbon.connector/src/test/resources/keystores/products/" folders.
 ...
 6. Make sure that planyo is specified as a module in ESB_Connector_Parent pom.
        <module>planyo/planyo-connector/planyo-connector-1.0.0/org.wso2.carbon.connector</module>

 7. Set up the Planyo account as mentioned below.
		...
		ii) Add a new user to the account.
			  ...
			  - Add the user by giving values for the mandatory fields. Save the user email address to be used in step 8(iii).
		...
		iii)Add a new resource to the system and add custom properties to the created resource.
			  ...
			  - Add the resource by giving required details. Retrieve the resource id from the url and save it to be used in step 8(iv).
			  ...
			  - Navigate to the created resource from Settings -> <Resource_Name> -> General info and provide a value for the added property. Save the property name and the value given later to be used in 8(v) and 8(vi) steps.
			  
		iv) Create a new Reservation by Navigating to Reservations -> Make Reservation and selecting a Resource. Then complete the wizard and make sure to include the user created in 7(iii) as the client. You can use the provided 'findClient' option to find the created User.
		...
		v)	Make sure to create at least one voucher by using following steps.
				...
				- Make sure to assign this voucher to created resource in step 7(iii).
		
 8. Update the properties in 'planyo.properties' file at location "{PLANYO_CONNECTOR_HOME}/planyo-connector/planyo-connector-1.0.0/org.wso2.carbon.connector/src/test/resources/artifacts/ESB/connector/config" as below.
	
	i) 		apiUrl 		          - Use the API URL as "http://api.planyo.com".
	ii)		apiKey 		          - Provide the Planyo API key.
	iii) 	email		          - Use the email address of the added user in step 7(ii).
	iv)		baseResourceId  	  - Use the ID of the added resource saved in step 7(iii).
	v)		resourcePropertyName  - Use the added custom property name for the resource saved in step 7(iii).
	vi)		resourcePropertyValue - Use the added custom property value for the resource saved in step 7(iii).
