...
public class GoogledriveConnectorIntegrationTest extends ESBIntegrationTest {
	...
	@Test(groups = { "wso2.esb" }, description = "Google Drive Integration Tests.")
	public void testGoogleDrive() throws Exception {
		String[] methodsToTest = googleDriveConnectorProperties.get("methodsToTest").toString().split(",");
		...
		for ( int i = 0; i < methodsToTest.length; i++ ) {
			...
			proxyAdmin.addProxyService(new DataHandler(new URL("file:"
					...
					+ ConnectorIntegrationUtil.ESB_CONFIG_LOCATION + File.separator + "proxies"
					+ File.separator + CONNECTOR_NAME + File.separator
			...
			OMElement getRequest = AXIOMUtil
					...
					+ ProductConstant.SYSTEM_TEST_RESOURCE_LOCATION
					+ ConnectorIntegrationUtil.ESB_CONFIG_LOCATION + File.separator + "soapRequests"
					+ File.separator + CONNECTOR_NAME + File.separator 
					+ methodName + ".xml", Charset.defaultCharset()));
			OMElement response = axisServiceClient.sendReceive(getRequest,
					getProxyServiceURL(CONNECTOR_NAME + "_" + methodName), "mediate");
