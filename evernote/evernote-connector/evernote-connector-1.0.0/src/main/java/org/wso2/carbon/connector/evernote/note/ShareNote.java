    ...
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            ...
            if(guid!=null&& !guid.trim().equalsIgnoreCase("")){
                String secretNoteKey = noteStoreClient.shareNote(guid);
                OMElement omResponse = EvernoteUtil.parseResponse("note.share.success");
                EvernoteUtil.addElement(omResponse,"secretNoteKey",secretNoteKey);
                EvernoteUtil.preparePayload(messageContext,omResponse);
                log.auditLog("Stop : shareNote");
            }
        } catch (TException e) {
