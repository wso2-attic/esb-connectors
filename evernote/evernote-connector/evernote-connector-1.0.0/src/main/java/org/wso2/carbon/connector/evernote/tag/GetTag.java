...
public class GetTag extends AbstractConnector {
    @Override
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            SynapseLog log = getLog(messageContext);
            log.auditLog("Start : GetTag");
            ...
            if(guid!=null&&!guid.equalsIgnoreCase("")) {
                ...
                OMElement omResponse = EvernoteUtil.parseResponse("tag.get.success");
                OMElement omTag = EvernoteUtil.createOMElement("tag");
                EvernoteUtil.addAttribute(omTag,"guid",tag.getGuid());
                EvernoteUtil.addElement(omTag,"name",tag.getName());
                EvernoteUtil.addAttribute(omTag,"parentGuid",tag.getParentGuid());
                EvernoteUtil.preparePayload(messageContext,omResponse);
                log.auditLog("Stop : GetTag");
            }
        } catch (TException e) {
