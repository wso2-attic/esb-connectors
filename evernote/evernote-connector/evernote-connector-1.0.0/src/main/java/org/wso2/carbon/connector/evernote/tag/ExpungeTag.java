...
public class ExpungeTag extends AbstractConnector {
    @Override
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            SynapseLog log = getLog(messageContext);
            log.auditLog("Start : ExpungeTag");
            ...
            if(guid!=null&&!guid.trim().equalsIgnoreCase("")){
                int updateSequenceNumber = noteStoreClient.expungeTag(guid);
                OMElement omResponse = EvernoteUtil.parseResponse("tag.delete.completely");
