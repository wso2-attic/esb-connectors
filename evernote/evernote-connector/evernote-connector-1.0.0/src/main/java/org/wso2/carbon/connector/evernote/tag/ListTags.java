...
public class ListTags extends AbstractConnector {
    @Override
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            SynapseLog log = getLog(messageContext);
            log.auditLog("Start : ListTags");
            NoteStoreClient noteStoreClient = EvernoteUtil.getNoteStoreClient(messageContext);
            List<Tag> tags = noteStoreClient.listTags();
            OMElement omResponse = EvernoteUtil.parseResponse("tag.listAll.success");
            OMElement omTags = EvernoteUtil.createOMElement("tags");
            for(Tag t : tags){
                OMElement omTag = EvernoteUtil.createOMElement("tag");
                EvernoteUtil.addAttribute(omTag,"guid",t.getGuid());
                EvernoteUtil.addElement(omTag,"name",t.getName());
                EvernoteUtil.addElement(omTag,"parentGuid",t.getGuid());
                omTags.addChild(omTag);
            }
            omResponse.addChild(omTags);
            EvernoteUtil.preparePayload(messageContext,omResponse);
            log.auditLog("Stop : ListTags");
        }  catch (TException e) {
