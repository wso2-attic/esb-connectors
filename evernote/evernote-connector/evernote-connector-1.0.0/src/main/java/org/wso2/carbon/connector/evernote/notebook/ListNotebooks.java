...
public class ListNotebooks extends AbstractConnector {
    @Override
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            SynapseLog log = getLog(messageContext);
            log.auditLog("Start : listNotebooks");
            ...
            OMElement omResponse = EvernoteUtil.parseResponse("notebooks.display.success");
            OMElement omNotebooks = EvernoteUtil.createOMElement("notebooks");

            for (Notebook notebook : notebooks){
                OMElement omNotebook = EvernoteUtil.createOMElement("notebook");
                EvernoteUtil.addAttribute(omNotebook,"guid",notebook.getGuid());
                EvernoteUtil.addElement(omNotebook,"notebookName",notebook.getName());
                omNotebooks.addChild(omNotebook);
            }
            omResponse.addChild(omNotebooks);
            EvernoteUtil.preparePayload(messageContext,omResponse);
            log.auditLog("Stop : listNotebooks");
        } catch (TException e) {
