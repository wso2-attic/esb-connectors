...
public class ExpungeSearch extends AbstractConnector {
    @Override
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            SynapseLog log = getLog(messageContext);
            log.auditLog("Start : expungeSearch");
            ...
            if(searchGuid!=null&&!searchGuid.trim().equalsIgnoreCase("")) {
                int updateSequenceNumber = noteStoreClient.expungeSearch(searchGuid);
                OMElement omResponse = EvernoteUtil.parseResponse("search.delete.success");
                ...
                log.auditLog("Stop : expungeSearch");
            }
        }  catch (TException e) {
