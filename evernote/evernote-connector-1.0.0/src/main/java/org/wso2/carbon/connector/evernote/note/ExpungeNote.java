    ...
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            ...
            if(guid!=null&&!guid.trim().equalsIgnoreCase("")){
                int update_sequence_number = noteStoreClient.expungeNote(guid);
                OMElement omResponse = EvernoteUtil.parseResponse("note.delete.completely");
                EvernoteUtil.addElement(omResponse,"update_sequence_number",update_sequence_number+"");
                EvernoteUtil.preparePayload(messageContext,omResponse);
                log.auditLog("Stop : expungeNote");
            }
