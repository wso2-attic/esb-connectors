...
public class StopSharingNote extends AbstractConnector {
    @Override
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            SynapseLog log = getLog(messageContext);
            log.auditLog("Start : stopSharingNote");
            ...
            if(guid!=null&&!guid.trim().equalsIgnoreCase("")){
                noteStoreClient.stopSharingNote(guid);
                OMElement omResponse = EvernoteUtil.parseResponse("note.stop.share");
                EvernoteUtil.preparePayload(messageContext,omResponse);
                log.auditLog("Stop : stopSharingNote");
            }
        }  catch (TException e) {
