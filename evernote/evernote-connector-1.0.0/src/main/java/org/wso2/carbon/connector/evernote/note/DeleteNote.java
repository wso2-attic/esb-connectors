...
public class DeleteNote extends AbstractConnector {
    @Override
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            SynapseLog log = getLog(messageContext);
            log.auditLog("Start : noteDelete ");
            ...
            if (guid!=null&&!guid.trim().equalsIgnoreCase("")) {
                int update_sequence_number = noteStoreClient.deleteNote(guid);
                OMElement omResponse = EvernoteUtil.parseResponse("note.delete.success");
