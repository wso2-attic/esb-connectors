...
public class GetSearch extends AbstractConnector {
    @Override
    public void connect(MessageContext messageContext) throws ConnectException {
        try {
            SynapseLog log = getLog(messageContext);
            log.auditLog("Start : getSearch");
            ...
            if(searchGuid!=null&&!searchGuid.trim().equalsIgnoreCase("")){
                SavedSearch search = noteStoreClient.getSearch(searchGuid);
                OMElement omResponse = EvernoteUtil.parseResponse("get.search.success");
                OMElement omSearch = EvernoteUtil.createOMElement("search");
                EvernoteUtil.addAttribute(omSearch,"guid",search.getGuid());
                EvernoteUtil.addElement(omSearch,"name",search.getName());
                EvernoteUtil.addElement(omSearch,"query",search.getQuery());
                omResponse.addChild(omSearch);
                EvernoteUtil.preparePayload(messageContext,omResponse);
                log.auditLog("Stop : getSearch");
            }
