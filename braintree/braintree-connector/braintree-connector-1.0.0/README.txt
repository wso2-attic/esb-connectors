...
STEPS: 
 ...
 2. Obtain private key, public key and merchant Id from created Braintree account:
	i) 	 Use the created Braintree account in step (1).
	ii)	 login to the account and navigate to Account => My user => click API keys (Autherization section).
	iii) Copy private key, public key and merchant Id as authentication details. 
 ...
 3. Create Custom fields for user account.
	...
	iv) Copy the provided API names for step (5).
	
 4. Get master merchant account Id.
	i) Login to the created account in step(1) and navigate to Settings => Processing.
	ii) In the merchant account section click "View all Merchant Accounts".
	iii) Copy the token related to merchant account type = "Master Merchant Account".
 ...
 6. Update the Braintree properties file at location "{BRAINTREE_CONNECTOR_HOME}/braintree-connector/braintree-connector-1.0.0/org.wso2.carbon.connector/src/test/resources/artifacts/ESB/connector/config" as below.
	...
	i) 		privateKey - Use the private key you coppied from step (2) => (iii).
	ii) 	publicKey - Use the public key you coppied from step (2) => (iii).
	iii) 	merchantId - Use the merchant Id you coppied from step (2) => (iii).
	iv)     environment - Since you created account details for sandbox, keep the environment as SANDBOX. 
						  (Available environments are SANDBOX, DEVELOPMENT and PRODUCTION)
						  
	v) 		masterMerchantAccountId - Use the token you coppied in step(4) => (iii). 
	vi) 	timeOut - Currently Braintree create transaction API expected rate is 30 seconds. Please make sure timeOut >= 30000 (ms)
	...
	xv)		customerId - Valid customer register to Braintree via Braintree Control Panel. Visit: https://sandbox.braintreegateway.com/merchants/<Your Merchant ID>/customers/new
	...
	xviii)	Leave the remaining properties unchanged.
