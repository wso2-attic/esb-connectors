...
public abstract class AmazonSESMediatorBase extends AbstractConnector {
    ...
    private void buildPayload() {
        ...
        try {
            ...
            log.error(AmazonSESConstants.CONNECTOR_ERROR, uee);
            storeErrorResponseStatus(messageContext, uee, AmazonSESConstants.UNSUPPORTED_ENCORDING_ERROR_CODE);
            handleException(AmazonSESConstants.CONNECTOR_ERROR, uee, messageContext);
        }
        payload.deleteCharAt(payload.length() - 1);
        final String payloadString = payload.toString();
        
        // Sets the payload as a property
        messageContext.setProperty(AmazonSESConstants.ESB_REQUEST_PAYLOAD_SET, payloadString);
    }
    ...
    private void generateSignature() {
        ...
        authorizationHeader.append(AmazonSESConstants.API_AUTHORIZATION_HEADER_PREFIX).append(AmazonSESConstants.SPACE)
                ...
                .append((String) messageContext.getProperty(AmazonSESConstants.ESB_AWS_ACCESS_KEY_ID).toString())
