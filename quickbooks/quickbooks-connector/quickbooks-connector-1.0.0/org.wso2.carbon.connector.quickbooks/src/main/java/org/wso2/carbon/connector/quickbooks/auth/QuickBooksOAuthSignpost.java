...
/**
 * Class QuickBooksOAuthSignpost which helps to generate authentication header for QuickBooks WSO2 ESB
 * Connector.
 */
public final class QuickBooksOAuthSignpost extends AbstractConnector {
    ...
    public void connect(final MessageContext messageContext) {
        ...
        final String requestMethod =
                messageContext.getProperty(QuickBooksConstants.REQUEST_METHOD).toString().toUpperCase();
        final String requestParams = messageContext.getProperty(QuickBooksConstants.PARAMS).toString();
        
        String requestUrl =
                messageContext.getProperty(QuickBooksConstants.API_URL).toString()
                        + messageContext.getProperty(QuickBooksConstants.URI_REMAINDER).toString();
        ...
        final String consumerKey = messageContext.getProperty(QuickBooksConstants.CONSUMER_KEY).toString();
        final String consumerSecret = messageContext.getProperty(QuickBooksConstants.CONSUMER_SECRET).toString();
        final String accessToken = messageContext.getProperty(QuickBooksConstants.ACCESS_TOKEN).toString();
        final String accessTokenSecret = messageContext.getProperty(QuickBooksConstants.ACCESS_TOKEN_SECRET).toString();
        ...
        try {
            ...
            storeErrorResponseStatus(messageContext, omse, QuickBooksConstants.OAUTH_MESSAGE_SIGNER_EXCEPTION);
            ...
            storeErrorResponseStatus(messageContext, oefe, QuickBooksConstants.OAUTH_EXPECTATION_FAILED_EXCEPTION);
            ...
            storeErrorResponseStatus(messageContext, oce, QuickBooksConstants.OAUTH_COMMUNICATION_EXCEPTION);
