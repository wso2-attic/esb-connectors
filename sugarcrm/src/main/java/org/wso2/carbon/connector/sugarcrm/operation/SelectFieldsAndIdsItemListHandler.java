...
/**
 * Class mediator which helps to modify the pay load for <strong>select_fields and id</strong> item lists in
 * SugarCRM SOAP API.
 */
public class SelectFieldsAndIdsItemListHandler extends AbstractConnector {
    ...
    public final void connect(final MessageContext messageContext) {
        ...
        Iterator< ? > omElementIteratorIds = mainElement.getChildrenWithLocalName(IDS_TAG);
        
        SugarCRMUtil.handleItemListElement(omElementIteratorIds);
        
        Iterator< ? > omElementIteratorSelectFields = mainElement.getChildrenWithLocalName(SELECT_FIELDS_TAG);
        
        SugarCRMUtil.handleItemListElement(omElementIteratorSelectFields);
