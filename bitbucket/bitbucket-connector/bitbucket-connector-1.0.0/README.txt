...
STEPS:
 ...
 3. Create a bitbucket account.
	i) 	Using the URL "https://bitbucket.org/account/signup/" to create a bitbucket account. And store the username and password for further reference.
	ii)Create a repository and add an existing project to it using Git. Store the revision ID of the commit for further reference. Add at least one comment to this commit.
	iii) Create five pull requests in the repository mentioned in step 3. iii) without any conflicts and store the pullRequestIDs for further reference. Add a comment to one of the pull request.
	iv)  Create two new branches from master branch in the repository mentioned in step 3. iii) and store for further references. 
	v) Add two commits to one of the branches created under step 3 v)
 ...
 4. Update the bitbucket properties file at location "{BITBUCKET_CONNECTOR_HOME}/bitbucket-connector/bitbucket-connector-1.0.0/org.wso2.carbon.connector/src/test/resources/artifacts/ESB/connector/config/" as below.
		...
		ii) 	username - Use the username obtained under step 3 i)
		iii) 	password - Use the password used to access the bitbucket account created under step 3 i).
		iv) 	owner - Use the name of the account owner which is as same as the username.
		v) 		repoSlug - Use the identifier of the created repository in bitbucket account mentioned under Step 3 ii).
		...
		ix)		revision - A revision ID to retrieve a commit. Use the revision ID obtained from Step 3 ii).
		x)		branchOrTag - A branch name or tag ID to retrieve commits.Use the branch name or tag ID used in commit done under Step     3 ii).
		...
		xii)	pullRequestIdComment - A pullRequest Id with commits in that pullRequest.Use the pullRequstID obtained under Step 3 iii) which has the comment.
		xiii)	pullRequestId - A pullRequest Id without any conflicts.Use one of the pullRequestIDs created under Step 3 iii) which is not already used.
		xiv)	pullRequestId1 - A pullRequest Id without any conflicts.Use one of the pullRequestIDs created under Step 3 iii) which is not already used.
		xv)		pullRequestId2 - A pullRequest Id without any conflicts.Use one of the pullRequestIDs created under Step 3 iii) which is not already used.
		xvi)	pullRequestId3 - A pullRequest Id without any conflicts.Use one of the pullRequestIDs created under Step 3 iii) which is not already used.
		...
		xix)	updateBranch - A Branch name the pulRequest need to be updated. Use the branch created under Step 3 iv) in which there are no commits.
		xx)		commentRevisionId - A commentRevision Id to retrieve comments. Use the commit revision ID obtained in Step 3 ii).
		xxi)	sourceBranchName - A Branch name to put the created pullRequest. Use the branch created in step 3 iv) in which a commit added in Step 3 v). 
		...
		Note: -Following properties should be changed in each run: vi) vii) viii) xvii) xviii)
			  -Property value of 'updateBranch' should be changed between 'master' and branch that was created under Step 3 iv) in which there are no commits.
