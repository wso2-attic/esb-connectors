...
/**
 * This class has utility methods which are used in unit testing.
 */
public class ConnectorIntegrationUtil {

    public static final String ESB_CONFIG_LOCATION = "artifacts"
	    + File.separator + "ESB" + File.separator + "config";

    private static final Log log = LogFactory
	    .getLog(ConnectorIntegrationUtil.class);
	...
	uploadedFileItem.setDataHandler(new DataHandler(new URL("file:" + "///"
		+ repoLocation + "/" + strFileName)));
    ...
    /**
     * Returns the property file with the given name, as a {@link Properties}
     * instance.
     * 
     * @param fileName
     *            Property file name.
     * @return Properties.
     */
    public static Properties getConnectorConfigProperties(String fileName) {

	String connectorConfigFile = null;
	ProductConstant.init();
	try {
	    connectorConfigFile = ProductConstant.SYSTEM_TEST_SETTINGS_LOCATION
		    + File.separator + "artifacts" + File.separator + "ESB"
		    + File.separator + "connector" + File.separator + "config"
    ...
    /**
     * Returns the content of the request file with the given name.
     * 
     * @param requestName
     *            Name of the request
     * @return Content of the request file.
     */
    public static String getRequest(String requestName) {

	String requestFile = ProductConstant.SYSTEM_TEST_SETTINGS_LOCATION
		+ File.separator + "requests" + File.separator + requestName
		+ ".req";

	try {
	    return IOUtils.toString(new FileInputStream(new File(requestFile)));
	} catch (IOException e) {
	    log.error(String.format("Cannot read request from file %s",
		    requestFile));
	}
    ...
    /**
     * Builds and returns a MEP Client with an attachment in the method context.
     * 
     * @param endpoint
     ...
     */
    public static OperationClient buildMEPClientWithAttachment(
	    EndpointReference endpoint, OMElement request,
	    Map<String, DataHandler> attachmentMap) throws AxisFault {

	ServiceClient serviceClient = new ServiceClient();

	Options serviceOptions = new Options();
	serviceOptions.setProperty(Constants.Configuration.ENABLE_SWA,
	...
	for (String contentId : attachmentMap.keySet()) {
	    messageContext.addAttachment(contentId,
    ...
    /**
     * Builds and returns a MEP with a specified soap envelope.
     * 
     * @param endpoint
     *            The endpoint to configure the client for.
     * @param request
